...
function validate(name: string, email: string, password: string, confirm: string): FieldError {
    ...
    else if (!/[A-Z]/.test(password) && !/[0-9]/.test(password)) errs.password = "Add at least one uppercase letter or number.";
...
const STRENGTH_TEXT = ["", "text-red-500", "text-amber-500", "text-yellow-500", "text-green-500"];
...
export default function RegisterPage() {
    ...
    const inputClass = (field: string) =>
        `w-full px-4 py-3 bg-slate-50 dark:bg-white/5 border rounded-xl text-sm text-slate-900 dark:text-white placeholder-slate-400 outline-none transition-all ${
            touched[field] && (fieldErrors as Record<string, string>)[field]
            ...
            >
                <div className="bg-white dark:bg-[#111118] border border-slate-200 dark:border-white/8 rounded-3xl p-8 shadow-2xl shadow-slate-200/80 dark:shadow-black/60">
                    ...
                    <div className="text-center mb-8">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Join thousands of tech enthusiasts</p>
                    </div>
                    ...
                    <motion.button
                        ...
                        className="w-full flex items-center justify-center gap-3 py-3 px-4 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-white/10 rounded-2xl text-sm font-semibold text-slate-700 dark:text-white transition-all shadow-sm disabled:opacity-60 mb-5"
                    >
                        {googleLoading ? (
                    ...
                    <div className="relative mb-5">
                        ...
                        <div className="relative flex justify-center text-xs">
                            <span className="bg-white dark:bg-[#111118] px-3 text-slate-400">or register with email</span>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} noValidate className="space-y-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                type="text"
                                value={name}
                                onChange={e => { setName(e.target.value); if (touched.name) setFieldErrors(validate(e.target.value, email, password, confirm)); }}
                        ...
                        <div>
                            ...
                            <input
                                ...
                                type="email"
                                value={email}
                                onChange={e => { setEmail(e.target.value); if (touched.email) setFieldErrors(validate(name, e.target.value, password, confirm)); }}
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    value={password}
                                    onChange={e => { setPassword(e.target.value); if (touched.password) setFieldErrors(validate(name, email, e.target.value, confirm)); }}
                            ...
                            {password && (
                                <div className="mt-2.5 space-y-1">
                                    <div className="flex gap-1">
                                        {[1, 2, 3, 4].map(s => (
                                            <div key={s} className={`h-1 flex-1 rounded-full transition-all duration-300 ${s <= strength ? STRENGTH_COLOR[strength] : "bg-slate-200 dark:bg-white/10"}`} />
                                        ))}
                                    </div>
                                    <p className={`text-[11px] font-medium ${STRENGTH_TEXT[strength]}`}>{STRENGTH_LABEL[strength]} password</p>
                                </div>
                            )}
                        ...
                        {/* Confirm password */}
                        <div>
                            <label htmlFor="reg-confirm" className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Confirm Password</label>
                            <div className="relative">
                                <input
                                    id="reg-confirm"
                                    type={showConfirm ? "text" : "password"}
                                    value={confirm}
                                    onChange={e => { setConfirm(e.target.value); if (touched.confirm) setFieldErrors(validate(name, email, password, e.target.value)); }}
                                    onBlur={() => handleBlur("confirm")}
                                    placeholder="Re-enter password"
                                    className={`${inputClass("confirm")} pr-11`}
                                />
                                <button type="button" onClick={() => setShowConfirm(v => !v)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                                        {showConfirm
                                            ? <path strokeLinecap="round" strokeLinejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
                                            : <><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></>
                        ...
                        <p className="text-center text-[11px] text-slate-400 mt-1">
                            ...
                            <Link href="#" className="text-red-500 hover:underline">Terms</Link> and{" "}
                            <Link href="#" className="text-red-500 hover:underline">Privacy Policy</Link>.
                        </p>
                    </form>

                    <p className="text-center text-sm text-slate-500 dark:text-slate-400 mt-5">
                        Already have an account?{" "}
                        <Link href="/login" className="text-red-500 hover:text-red-600 dark:hover:text-red-400 font-semibold transition-colors">
                            Sign in →
                        </Link>
                    </p>
                </div>
